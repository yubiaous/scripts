...
Deposit_CallHistory
	[Documentation]		"Deposit CallHistory message"
	[Tags]				PM_OEM_MO	Critical
    [Setup]         prepare_testbed
    [Teardown]      custom_teardown
    ${response}=    Deposit_Message       headers=${CH_DEPOSIT_HEADERS}	  object_file=${CALLHISTORY_DEPOSIT_OBJ_FILE}	url=${HOST_NMS_URI}		DIRECTION_VALUE=Out
	...
	${pns_status}	${value}=		Run Keyword and Ignore Error		ValidateCallHistoryPNSNotfn		pns_headers=${headers}	pns_body=${req_body}	uid=${CALLHISTORY_UID}	
    ...
    Run Keyword and Continue on Failure     ValidateCassandraForCallHistory		userId=${USERID}	answered=1
...
FolderSearchAfterDeposit
	...
	ValidateCallHistoryFolderSearchResponse		response=${response}	msgStatus=\\${CALL_STATUS}			uid=${CALLHISTORY_UID}
...
Fetch_CallLog
	...
	ValidateCallHistoryFetchResponse	response=${response}	msgStatus=\\${CALL_STATUS}		uid=${CALLHISTORY_UID}
    Run Keyword and Continue on Failure     Peg_TMM_file_immediately
    Sleep   10
...
Update_Msg_from_Seen_to_Recent
    ...
    ${request}=     GetServicerequest  ${PNS_SOCKET_SERVICE}
    ${headers}      ${req_body}     ${version}=     GetRequestData  ${request}
    ${pns_status}   ${value}=       Run Keyword and Ignore Error        ValidateUpdateCallHistoryPNSNotfn 	pns_headers=${headers}  	pns_body=${req_body}	msgStatus=${CALL_STATUS},recent		pns_subtype=HistoryS		uid=${CALLHISTORY_UID}
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "PASS"   SendResponse    ${request}  200     OK  ${version}  ELSE    SendResponse    ${request}  400     BadRequest   ${version}
    Run Keyword and Continue on Failure     ValidateCassandraForCallHistory     userId=${USERID}	answered=1	recent=1	uid=0	
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "FAIL"   FAIL    PNS notification verification failed \n ${value}
	CloseRequest    ${request}
...
FolderSearchAfterUpdatingtheFlagtoRecent
    ...
    ${response}=    FolderSearch        folderkey=${CALLHISTORY_PARENT_FOLDER_KEY}      headers=${OEM_FOLDER_SEARCH_HEADER}	
    ValidateCallHistoryFolderSearchResponse     response=${response}	msgStatus=\\recent,\\${CALL_STATUS}		uid=${CALLHISTORY_UID}	
...
Update_Msg_from_Recent_to_Seen
    ...
    ${request}=     GetServicerequest  ${PNS_SOCKET_SERVICE}
    ${headers}      ${req_body}     ${version}=     GetRequestData  ${request}
    ${pns_status}   ${value}=       Run Keyword and Ignore Error        ValidateUpdateCallHistoryPNSNotfn 	pns_headers=${headers}		pns_body=${req_body}	msgStatus=${CALL_STATUS},seen	uid=${CALLHISTORY_UID}	
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "PASS"   SendResponse    ${request}  200     OK  ${version}  ELSE    SendResponse    ${request}  400     BadRequest   ${version}
    Run Keyword and Continue on Failure     ValidateCassandraForCallHistory     userId=${USERID}	answered=1	seen=1	recent=0		uid=0
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "FAIL"   FAIL    PNS notification verification failed \n ${value}
	CloseRequest    ${request}
...
FolderSearchAfterUpdatingtheFlagtoSeen
    ...
    [Teardown]      custom_teardown
    ${response}=    FolderSearch        folderkey=${CALLHISTORY_PARENT_FOLDER_KEY}      headers=${OEM_FOLDER_SEARCH_HEADER}
    ValidateCallHistoryFolderSearchResponse     response=${response}	msgStatus=\\seen,\\${CALL_STATUS}		uid=${CALLHISTORY_UID}
...
Delete_CallLog
   ...
   # Run Keyword If  ${EnableDeltraSync} == False     Run Keyword and Continue on Failure     ValidateCassandraMessagesAfterDelete    messages=0  messages_by_folder_timestamp=0		parent_folder_key=${CALLHISTORY_PARENT_FOLDER_KEY}  message_folder_key=${CALLHISTORY_PARENT_FOLDER_KEY}
...
FolderSearchAfterDeletingtheMsg
    ...
    [Setup]         prepare_testbed
    [Teardown]      custom_teardown
    ${response}=    FolderSearch        folderkey=${CALLHISTORY_PARENT_FOLDER_KEY}      headers=${OEM_FOLDER_SEARCH_HEADER}		response_code=200
    Run Keyword and Continue on Failure		ValidateCallHistoryFolderSearchResponse     response=${response}	msgStatus=\\deleted		uid=${CALLHISTORY_UID}
