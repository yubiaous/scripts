...
Test Timeout	2m
Documentation	"Digits FT MO Flow"
...
#	Run Keyword and Continue on Failure     ValidateCassandraMessagesTable  recent=0    seen=1		uid=${uid}	messagecontext=${FT_SESSION_MESSAGE_CONTEXT}	userId=${USERID}
...
Deposit_FT_Message
	...
	log		${PNS_TYPE}		console=true
	${pns_status}	${value}=		Run Keyword and Ignore Error		ValidateFTPNSNotfn	pns_headers=${headers}	pns_body=${req_body}	direction=${DIRECTION}	msgStatus=SEEN	push_recipients_uri=${push_recepient_uri}	pns_subtype=FileTransferO			
...
#    Run Keyword and Continue on Failure     ValidateCassandraMessagesTable  recent=0    seen=1	uid=${uid}	
...
FolderSearchAfterDeposit
	...
	Run Keyword and Continue on Failure		ValidateFTFolderSearchResponse_along_with_Session_FT	direction=${DIRECTION}	response=${response}	msgStatus=seen	session_msgStatus=seen	#P_ASSERTED_SERVICE=${P_ASSERTED_SERVICE_FT}
...
Deliver_FT_Message
    ...
    [Setup]         prepare_testbed
    [Teardown]      custom_teardown
    Deposit_Message		X_RCS_MSG_STATUS=Delivered	object_file=${DELIVER_DISPLAY_OBJ_FILE}		MESSAGE_CONTEXT=X-RCS-IMDN	 DIRECTION_VALUE=In		headers=${OEM_DEPOSIT_HEADERS}		#P_ASSERTED_SERVICE=${P_ASSERTED_SERVICE_FT}
    ${request}=     GetServicerequest  ${PNS_SOCKET_SERVICE}
    ${headers}      ${req_body}     ${version}=     GetRequestData  ${request}
    ${pns_status}   ${value}=     Run Keyword and Ignore Error	ValidateFTPNSNotfn        pns_headers=${headers}  pns_body=${req_body}	msgStatus=DELIVERED		direction=${DIRECTION}	push_recipients_uri=${push_recepient_uri}		pns_subtype=FileTransferS 		uid=${ft_deposit_uid}			imdn_pns=delivered		#P_ASSERTED_SERVICE=${P_ASSERTED_SERVICE_FT}
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "PASS"   SendResponse    ${request}  200     OK  ${version}  ELSE    SendResponse    ${request}  400     BadRequest   ${version}
    ${creation_tuid}    ${modification_tuid}=   Run Keyword and Continue on Failure     ValidateCassandra_messages_by_original_folder_timestamp_LargeMessages   recent=0    seen=1	 deliveredimdnlist=${MSG_TO}
    Run Keyword and Continue on Failure     ValidateCassandra_messages_by_root_folder_timestamp     recent=0    seen=1	modification_tuid=${modification_tuid}
    Run Keyword and Continue on Failure     ValidateCassandra_flag_changes_by_root_folder_timestamp     seen=1		modification_tuid=${modification_tuid}
    Run Keyword and Continue on Failure     ValidateCassandra_flag_changes_by_original_folder_timestamp     seen=1	 modification_tuid=${modification_tuid}
    Run Keyword and Continue on Failure     ValidateCassandraIMDNMapping    uid=${CREATION_TUID}
    Run Keyword and Continue on Failure     ValidateCassandraMessageActivity	cnt=1

#	Run Keyword and Continue on Failure        ValidateCassandraMessagesTable	delivered=0		recent=0	seen=1	uid=${ft_deposit_uid}				delivered_imdn_list=${MSG_TO}   	
#    Run Keyword and Continue on Failure     ValidateCassandraIMDNMapping	uid=${ft_deposit_uid}
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "FAIL"   FAIL	PNS notification verification failed \n ${value}
	CloseRequest    ${request}
...
FolderSearchAfterIMDNDelivered
    ...
    Run Keyword and Continue on Failure     ValidateFTFolderSearchResponse_along_with_Session_FT    direction=${DIRECTION}  response=${response}    msgStatus=seen  session_msgStatus=seen	 imdn_type=delivered
...
Display_FT_Message
    ...
    ${request}=     GetServicerequest  ${PNS_SOCKET_SERVICE}
    ${headers}      ${req_body}     ${version}=     GetRequestData  ${request}
    ${pns_status}   ${value}=     Run Keyword and Ignore Error	ValidateFTPNSNotfn        pns_headers=${headers}  pns_body=${req_body}	msgStatus=SEEN		direction=${DIRECTION}	push_recipients_uri=${push_recepient_uri}		pns_subtype=FileTransferS		uid=${ft_deposit_uid}			imdn_pns=displayed
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "PASS"   SendResponse    ${request}  200     OK  ${version}  ELSE    SendResponse    ${request}  400     BadRequest   ${version}
    ${creation_tuid}    ${modification_tuid}=   Run Keyword and Continue on Failure     ValidateCassandra_messages_by_original_folder_timestamp_LargeMessages   recent=0    seen=1   deliveredimdnlist=${MSG_TO}	readimdnlist=${MSG_TO}
    Run Keyword and Continue on Failure     ValidateCassandra_messages_by_root_folder_timestamp     recent=0    seen=1  modification_tuid=${modification_tuid}
    Run Keyword and Continue on Failure     ValidateCassandra_flag_changes_by_root_folder_timestamp     seen=1      modification_tuid=${modification_tuid}
    Run Keyword and Continue on Failure     ValidateCassandra_flag_changes_by_original_folder_timestamp     seen=1   modification_tuid=${modification_tuid}
    Run Keyword and Continue on Failure     ValidateCassandraIMDNMapping    uid=${CREATION_TUID}
...
#    Run Keyword and Continue on Failure        ValidateCassandraMessagesTable       delivered=0		seen=1	recent=0	uid=${ft_deposit_uid}     		delivered_imdn_list=${MSG_TO}   read_imdn_list=${MSG_TO}	
#    Run Keyword and Continue on Failure     ValidateCassandraIMDNMapping	uid=${ft_deposit_uid}
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "FAIL"   FAIL	PNS notification verification failed \n ${value}
	CloseRequest    ${request}
...
FolderSearchAfterIMDNArchival
    ...
    [Teardown]      custom_teardown
	${response}=    FolderSearch	headers=${OEM_FOLDER_SEARCH_HEADER}
    Run Keyword and Continue on Failure     ValidateFTFolderSearchResponse_along_with_Session_FT    direction=${DIRECTION}  response=${response}    msgStatus=seen  session_msgStatus=seen	  imdn_type=displayed
...
Fetch_FT_Message
	...
	ValidateFTFetchResponse	response=${response}	msgStatus=seen		direction=${DIRECTION}	uid=${ft_deposit_uid}
...
Update_Msg_from_Seen_to_Recent
    ...
    ${request}=     GetServicerequest  ${PNS_SOCKET_SERVICE}
    ${headers}      ${req_body}     ${version}=     GetRequestData  ${request}
    ${pns_status}   ${value}=     Run Keyword and Ignore Error	ValidateFTPNSNotfn     pns_headers=${headers}  pns_body=${req_body}	direction=${DIRECTION}     msgStatus=RECENT	push_recipients_uri=${push_recepient_uri}		pns_subtype=FileTransferS		uid=${ft_deposit_uid}	 
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "PASS"   SendResponse    ${request}  200     OK  ${version}  ELSE    SendResponse    ${request}  400     BadRequest   ${version}
    ${creation_tuid}    ${modification_tuid}=   Run Keyword and Continue on Failure     ValidateCassandra_messages_by_original_folder_timestamp_LargeMessages   recent=1    seen=0   deliveredimdnlist=${MSG_TO}    readimdnlist=${MSG_TO}
    Run Keyword and Continue on Failure     ValidateCassandra_messages_by_root_folder_timestamp     recent=1    seen=0  modification_tuid=${modification_tuid}
    Run Keyword and Continue on Failure     ValidateCassandra_flag_changes_by_root_folder_timestamp     seen=0      modification_tuid=${modification_tuid}
    Run Keyword and Continue on Failure     ValidateCassandra_flag_changes_by_original_folder_timestamp     seen=0   modification_tuid=${modification_tuid}
    Run Keyword and Continue on Failure     ValidateCassandraIMDNMapping    uid=${CREATION_TUID}
    Run Keyword and Continue on Failure     ValidateCassandraMessageActivity    cnt=2

#    Run Keyword and Continue on Failure     ValidateCassandraMessagesTable	recent=1	delivered=0		uid=${ft_deposit_uid}			 delivered_imdn_list=${MSG_TO}   read_imdn_list=${MSG_TO}
#    Run Keyword and Continue on Failure     ValidateCassandraIMDNMapping	uid=${ft_deposit_uid}
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "FAIL"   FAIL	PNS notification verification failed \n ${value}
	CloseRequest    ${request}
...
FolderSearchAfterUpdatingtheFlagtoRecent
    ...
    [Teardown]      custom_teardown
    ${response}=    FolderSearch	headers=${OEM_FOLDER_SEARCH_HEADER}
    Run Keyword and Continue on Failure     ValidateFTFolderSearchResponse_along_with_Session_FT    direction=${DIRECTION}  response=${response}    msgStatus=recent  session_msgStatus=seen	imdn_type=displayed
...
Update_Msg_from_Recent_to_Seen
    ...
    ${request}=     GetServicerequest  ${PNS_SOCKET_SERVICE}
    ${headers}      ${req_body}     ${version}=     GetRequestData  ${request}
	${pns_status}   ${value}=     Run Keyword and Ignore Error	ValidateFTPNSNotfn    pns_headers=${headers}  pns_body=${req_body}    msgStatus=SEEN	direction=${DIRECTION}	push_recipients_uri=${push_recepient_uri}		pns_subtype=FileTransferS 		uid=${ft_deposit_uid}	
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "PASS"   SendResponse    ${request}  200     OK  ${version}  ELSE    SendResponse    ${request}  400     BadRequest   ${version}
    ${creation_tuid}    ${modification_tuid}=   Run Keyword and Continue on Failure     ValidateCassandra_messages_by_original_folder_timestamp_LargeMessages   recent=0    seen=1   deliveredimdnlist=${MSG_TO}    readimdnlist=${MSG_TO}
    Run Keyword and Continue on Failure     ValidateCassandra_messages_by_root_folder_timestamp     recent=0    seen=1  modification_tuid=${modification_tuid}
    Run Keyword and Continue on Failure     ValidateCassandra_flag_changes_by_root_folder_timestamp     seen=1      modification_tuid=${modification_tuid}
...
#	Run Keyword and Continue on Failure     ValidateCassandraMessagesTable	seen=1  recent=0	delivered=0		uid=${ft_deposit_uid}			 delivered_imdn_list=${MSG_TO}   read_imdn_list=${MSG_TO}
#    Run Keyword and Continue on Failure     ValidateCassandraIMDNMapping	uid=${ft_deposit_uid}
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "FAIL"   FAIL	PNS notification verification failed \n ${value}
	CloseRequest    ${request}
...
DeleteFTMessage
    ...
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "PASS"   SendResponse    ${request}  200     OK  ${version}  ELSE    SendResponse    ${request}  400     BadRequest   ${version}
    ${creation_tuid}    ${modification_tuid}=   Run Keyword and Continue on Failure     ValidateCassandra_messages_by_original_folder_timestamp_LargeMessages   recent=0    seen=0   deliveredimdnlist=${MSG_TO}    readimdnlist=${MSG_TO}	deleted=1
    Run Keyword and Continue on Failure     ValidateCassandra_messages_by_root_folder_timestamp     recent=0    seen=0  modification_tuid=${modification_tuid}	deleted=1
    Run Keyword and Continue on Failure     ValidateCassandra_flag_changes_by_root_folder_timestamp     seen=0      modification_tuid=${modification_tuid}	deleted=1
    Run Keyword and Continue on Failure     ValidateCassandra_flag_changes_by_original_folder_timestamp     seen=0   modification_tuid=${modification_tuid}		deleted=1
    Run Keyword and Continue on Failure     ValidateCassandraIMDNMapping    uid=${CREATION_TUID}
    Run Keyword and Continue on Failure     ValidateCassandraMessageActivity    cnt=2
...
FolderSearchAfterDeletingtheMsg
    ...
    Run Keyword If      ${EnableDeltraSync} == True     Run Keyword and Continue on Failure     ValidateFTFolderSearchResponse_along_with_Session_FT    response=${response}    msgStatus=Deleted   direction=${DIRECTION}  session_msgStatus=seen        no_of_msgs=${no_of_msgs}	 imdn_type=displayed
...
FolderSearchAfterDeletingSession_and_Msg
    [Tags]              FT   Critical
    [Setup]         prepare_testbed
    [Teardown]      custom_teardown
...
#    Run Keyword If      ${EnableDeltraSync} == True     Run Keyword and Continue on Failure     ValidateFTFolderSearchResponse_along_with_Session_FT    response=${response}    msgStatus=Deleted   direction=${DIRECTION}  session_msgStatus=recent        no_of_msgs=2
