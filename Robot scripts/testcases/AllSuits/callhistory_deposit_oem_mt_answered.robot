...
Deposit_CallHistory
    ...
    [Setup]         prepare_testbed
    [Teardown]      custom_teardown
    ${response}=    Deposit_Message       headers=${CH_DEPOSIT_HEADERS}	  object_file=${CALLHISTORY_DEPOSIT_OBJ_FILE}	url=${HOST_NMS_URI}
	...
	${pns_status}	${value}=		Run Keyword and Ignore Error		ValidateCallHistoryPNSNotfn		pns_headers=${headers}	pns_body=${req_body}		uid=${UID}	
    ...
    Run Keyword and Continue on Failure     ValidateCassandraForCallHistory		answered=1
...
FolderSearchAfterDeposit
	...
	ValidateCallHistoryFolderSearchResponse		response=${response}	msgStatus=\\${CALL_STATUS}
...
Fetch_CallLog
	...
	ValidateCallHistoryFetchResponse	response=${response}	msgStatus=\\${CALL_STATUS}		uid=${UID}
...
Update_Msg_from_Seen_to_Recent
    ...
    ${request}=     GetServicerequest  ${PNS_SOCKET_SERVICE}
    ${headers}      ${req_body}     ${version}=     GetRequestData  ${request}
    ${pns_status}   ${value}=       Run Keyword and Ignore Error        ValidateUpdateCallHistoryPNSNotfn 	pns_headers=${headers}  pns_body=${req_body}	msgStatus=${CALL_STATUS},recent			uid=${UID}		pns_subtype=HistoryS
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "PASS"   SendResponse    ${request}  200     OK  ${version}  ELSE    SendResponse    ${request}  400     BadRequest   ${version}
    Run Keyword and Continue on Failure     ValidateCassandraForCallHistory     answered=1	recent=1		uid=0
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "FAIL"   FAIL    PNS notification verification failed \n ${value}
    CloseRequest    ${request}
...
FolderSearchAfterUpdatingtheFlagtoRecent
    ...
    ${response}=    FolderSearch        folderkey=${CALLHISTORY_PARENT_FOLDER_KEY}      headers=${OEM_FOLDER_SEARCH_HEADER}	
    ValidateCallHistoryFolderSearchResponse     response=${response}	msgStatus=\\recent,\\${CALL_STATUS}
...
Update_Msg_from_Recent_to_Seen
    ...
    ${request}=     GetServicerequest  ${PNS_SOCKET_SERVICE}
    ${headers}      ${req_body}     ${version}=     GetRequestData  ${request}
    ${pns_status}   ${value}=       Run Keyword and Ignore Error        ValidateUpdateCallHistoryPNSNotfn 	pns_headers=${headers}  pns_body=${req_body}	msgStatus=${CALL_STATUS},seen		uid=${UID}
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "PASS"   SendResponse    ${request}  200     OK  ${version}  ELSE    SendResponse    ${request}  400     BadRequest   ${version}
    Run Keyword and Continue on Failure     ValidateCassandraForCallHistory     answered=1	seen=1	recent=0		uid=0
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "FAIL"   FAIL    PNS notification verification failed \n ${value}
	CloseRequest    ${request}
...
FolderSearchAfterUpdatingtheFlagtoSeen
    ...
    [Teardown]      custom_teardown
    ${response}=    FolderSearch        folderkey=${CALLHISTORY_PARENT_FOLDER_KEY}      headers=${OEM_FOLDER_SEARCH_HEADER}
    ValidateCallHistoryFolderSearchResponse     response=${response}	msgStatus=\\seen,\\${CALL_STATUS}
...
#	${request}=     GetServicerequest  ${PNS_SOCKET_SERVICE}
#    ${headers}      ${req_body}     ${version}=     GetRequestData  ${request}
#    ${pns_status}   ${value}=       Run Keyword and Ignore Error        ValidateCallHistoryDeletePNSNotfn 	pns_headers=${headers}  pns_body=${req_body} 	pns_subtype=HistoryD
 #   Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "PASS"   SendResponse    ${request}  200     OK  ${version}  ELSE    SendResponse    ${request}  400     BadRequest   ${version}
#	Run Keyword If	${EnableDeltraSync} == True		Run Keyword and Continue on Failure		ValidateCassandraMessagesAfterDelete	messages=0	messages_by_folder_timestamp=1	parent_folder_key=${CALLHISTORY_PARENT_FOLDER_KEY}	message_folder_key=${CALLHISTORY_PARENT_FOLDER_KEY}
