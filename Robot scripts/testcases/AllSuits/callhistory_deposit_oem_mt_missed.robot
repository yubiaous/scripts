...
Deposit_CallHistory
	...
	${pns_status}	${value}=		Run Keyword and Ignore Error		ValidateCallHistoryPNSNotfn		pns_headers=${headers}	pns_body=${req_body}	msgStatus=${CALL_STATUS1}	pns_subtype=History		 uid=${CALLHISTORY_UID}
    ...
    Run Keyword and Continue on Failure     ValidateCassandraForCallHistory		answered=0
...
FolderSearchAfterDeposit
	...
	ValidateCallHistoryFolderSearchResponse		response=${response}	msgStatus=\$${CALL_STATUS1}
...
Fetch_CallLog
	...
	ValidateCallHistoryFetchResponse	response=${response}	msgStatus=\$${CALL_STATUS1}		uid=${CALLHISTORY_UID}
...
Update_Msg_from_Seen_to_Recent
    ...
    ${request}=     GetServicerequest  ${PNS_SOCKET_SERVICE}
    ${headers}      ${req_body}     ${version}=     GetRequestData  ${request}
    ${pns_status}   ${value}=       Run Keyword and Ignore Error        ValidateUpdateCallHistoryPNSNotfn 	pns_headers=${headers}  pns_body=${req_body}	msgStatus=recent,missed		pns_subtype=History		uid=${CALLHISTORY_UID}		pns_subtype=HistoryS 
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "PASS"   SendResponse    ${request}  200     OK  ${version}  ELSE    SendResponse    ${request}  400     BadRequest   ${version}
...
FolderSearchAfterUpdatingtheFlagtoRecent
    ...
    ${response}=    FolderSearch        folderkey=${CALLHISTORY_PARENT_FOLDER_KEY}      headers=${OEM_FOLDER_SEARCH_HEADER}	
    ValidateCallHistoryFolderSearchResponse     response=${response}	msgStatus=\\recent,\$${CALL_STATUS1}
...
Update_Msg_from_Recent_to_Seen
    ...
    ${request}=     GetServicerequest  ${PNS_SOCKET_SERVICE}
    ${headers}      ${req_body}     ${version}=     GetRequestData  ${request}
    ${pns_status}   ${value}=       Run Keyword and Ignore Error        ValidateUpdateCallHistoryPNSNotfn 	pns_headers=${headers}  pns_body=${req_body}	msgStatus=seen,missed		uid=${CALLHISTORY_UID}			pns_subtype=HistoryS
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "PASS"   SendResponse    ${request}  200     OK  ${version}  ELSE    SendResponse    ${request}  400     BadRequest   ${version}
...
FolderSearchAfterUpdatingtheFlagtoSeen
    ...
    [Teardown]      custom_teardown
    ${response}=    FolderSearch        folderkey=${CALLHISTORY_PARENT_FOLDER_KEY}      headers=${OEM_FOLDER_SEARCH_HEADER}
    ValidateCallHistoryFolderSearchResponse     response=${response}	msgStatus=\\seen,\$${CALL_STATUS1}
