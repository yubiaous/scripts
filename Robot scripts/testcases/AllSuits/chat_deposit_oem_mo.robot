...
Metadata    	Version        MSTR-C.4.4_19.2
Suite Setup     prepare_suitebed
Suite Teardown  custom_suite_teardown
Test Timeout	2m
Documentation	"Digits Chat OEM MO Flow"
...
CreateSessionForChatDeposit
	...
	Run Keyword and Continue on Failure     ValidateCassandraMessagesTable_C44		userId=${USERID}	message_folder_key=${RCS_MESSAGE_FOLDER_KEY}		recent=0    seen=1		uid=0	messagecontext=${CHAT_SESSION_MESSAGE_CONTEXT}	creation_tuid=${UID}
...
Deposit_Chat_Message
	...
	${pns_status}	${value}=		Run Keyword and Ignore Error		ValidateChatPNSNotfn	pns_headers=${headers}	pns_body=${req_body}	direction=${DIRECTION}	msgStatus=SEEN	push_recipients_uri=${push_recepient_uri}	pns_subtype=ChatO		uid=${chat_deposit_uid}
...
#    Run Keyword and Continue on Failure     ValidateCassandraMessagesTable_C44   userId=${USERID}		message_folder_key=${RCS_MESSAGE_FOLDER_KEY}	recent=0    seen=1	uid=${chat_deposit_uid}	
    ${creation_tuid}    ${modification_tuid}=   Run Keyword and Continue on Failure     ValidateCassandra_messages_by_original_folder_timestamp_CHAT    recent=0    seen=1  delivered=0      
...
FolderSearchAfterDeposit
	...
	Run Keyword and Continue on Failure		ValidateChatFolderSearchResponse_along_with_Session_Chat	 direction=${DIRECTION}			response=${response}		uid=${UID}		session_msg_uid=${chat_deposit_uid}		msgStatus=seen	session_msgStatus=seen	
...
Deliver_Chat_Message
	...
	Set Test Variable   ${request}
    ${headers}      ${req_body}     ${version}=     GetRequestData  ${request}
    ${pns_status}   ${value}=     Run Keyword and Ignore Error	ValidateChatPNSNotfn        pns_headers=${headers}  pns_body=${req_body}	msgStatus=SEEN,DELIVERED		direction=${DIRECTION}	push_recipients_uri=${push_recepient_uri}	pns_subtype=ChatS	uid=${chat_deposit_uid}			imdn_pns=delivered	
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "PASS"   SendResponse    ${request}  200     OK  ${version}  ELSE    SendResponse    ${request}  400     BadRequest   ${version}
	#Run Keyword and Continue on Failure        ValidateCassandraMessagesTable_C44 	userId=${USERID}	delivered=0		recent=0	seen=1	uid=${chat_deposit_uid}				delivered_imdn_list=${MSG_TO}   	
	 ${creation_tuid}    ${modification_tuid}=   Run Keyword and Continue on Failure     ValidateCassandra_messages_by_original_folder_timestamp_CHAT    recent=0    seen=1  delivered=1      deliveredimdnlist=${MSG_TO}    
...
FolderSearchAfterIMDNDelivered
    ...
    Run Keyword and Continue on Failure     ValidateChatFolderSearchResponse_along_with_Session_Chat    direction=${DIRECTION}  response=${response}	uid=${UID}    msgStatus=seen  session_msgStatus=seen	 imdn_type=delivered		session_msg_uid=${chat_deposit_uid}
...
Display_Chat_Message
	...
	Set Test Variable   ${request}
    ${headers}      ${req_body}     ${version}=     GetRequestData  ${request}
    ${pns_status}   ${value}=     Run Keyword and Ignore Error	ValidateChatPNSNotfn        pns_headers=${headers}  pns_body=${req_body}	msgStatus=SEEN,DELIVERED		direction=${DIRECTION}	push_recipients_uri=${push_recepient_uri}	pns_subtype=ChatS		uid=${chat_deposit_uid}			imdn_pns=displayed
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "PASS"   SendResponse    ${request}  200     OK  ${version}  ELSE    SendResponse    ${request}  400     BadRequest   ${version}
#    Run Keyword and Continue on Failure        ValidateCassandraMessagesTable_C44      userId=${USERID}	 delivered=0		seen=1	recent=0	uid=${chat_deposit_uid}     		delivered_imdn_list=${MSG_TO}   read_imdn_list=${MSG_TO}
#	${creation_tuid}    ${modification_tuid}=   Run Keyword and Continue on Failure     ValidateCassandra_messages_by_original_folder_timestamp_CHAT    recent=0    seen=1  delivered=1     delivered_imdn_list=${MSG_TO}   read_imdn_list=${MSG_TO}
...
FolderSearchAfterIMDNArchival
    ...
    [Teardown]      custom_teardown
	${response}=    FolderSearch	headers=${OEM_FOLDER_SEARCH_HEADER}
    Run Keyword and Continue on Failure     ValidateChatFolderSearchResponse_along_with_Session_Chat  		 	direction=${DIRECTION}  response=${response} 		uid=${UID}	   msgStatus=seen  session_msgStatus=seen	  imdn_type=displayed		session_msg_uid=${chat_deposit_uid}
...
Fetch_Chat_Message
	...
	ValidateChatFetchResponse	response=${response}	msgStatus=seen		direction=${DIRECTION}	uid=${chat_deposit_uid} 
...
Update_Msg_from_Seen_to_Recent
	...
	Set Test Variable   ${request}
    ${headers}      ${req_body}     ${version}=     GetRequestData  ${request}
    ${pns_status}   ${value}=     Run Keyword and Ignore Error	ValidateChatPNSNotfn     pns_headers=${headers}  pns_body=${req_body}	direction=${DIRECTION}     msgStatus=RECENT,DELIVERED	push_recipients_uri=${push_recepient_uri}	pns_subtype=ChatS		uid=${chat_deposit_uid}		folder_path=${CHAT_PARENT_FOLDER_PATH}/${FROM_MSISDN}/
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "PASS"   SendResponse    ${request}  200     OK  ${version}  ELSE    SendResponse    ${request}  400     BadRequest   ${version}
#    Run Keyword and Continue on Failure     ValidateCassandraMessagesTable	recent=1	delivered=0		uid=${chat_deposit_uid}			 delivered_imdn_list=${MSG_TO}   read_imdn_list=${MSG_TO}
	# ${creation_tuid}    ${modification_tuid}=   Run Keyword and Continue on Failure     ValidateCassandra_messages_by_original_folder_timestamp_CHAT    recent=1    seen=0  delivered=1		delivered_imdn_list=${MSG_TO}   read_imdn_list=${MSG_TO}
...
FolderSearchAfterUpdatingtheFlagtoRecent
    ...
    [Teardown]      custom_teardown
    ${response}=    FolderSearch	headers=${OEM_FOLDER_SEARCH_HEADER}
    Run Keyword and Continue on Failure     ValidateChatFolderSearchResponse_along_with_Session_Chat  	 direction=${DIRECTION}  response=${response} 		uid=${UID}		   msgStatus=recent  session_msgStatus=seen	imdn_type=displayed			session_msg_uid=${chat_deposit_uid}
...
Update_Msg_from_Recent_to_Seen
	...
	Set Test Variable   ${request}
    ${headers}      ${req_body}     ${version}=     GetRequestData  ${request}
	${pns_status}   ${value}=     Run Keyword and Ignore Error	ValidateChatPNSNotfn    pns_headers=${headers}  pns_body=${req_body}    msgStatus=SEEN,DELIVERED	direction=${DIRECTION}	push_recipients_uri=${push_recepient_uri}	pns_subtype=ChatS		uid=${chat_deposit_uid}		folder_path=${CHAT_PARENT_FOLDER_PATH}/${FROM_MSISDN}/
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "PASS"   SendResponse    ${request}  200     OK  ${version}  ELSE    SendResponse    ${request}  400     BadRequest   ${version}
#	Run Keyword and Continue on Failure     ValidateCassandraMessagesTable	seen=1  recent=0	delivered=0		uid=${chat_deposit_uid}			 delivered_imdn_list=${MSG_TO}   read_imdn_list=${MSG_TO}
#     ${creation_tuid}    ${modification_tuid}=   Run Keyword and Continue on Failure     ValidateCassandra_messages_by_original_folder_timestamp_CHAT    recent=0    seen=1  delivered=1		delivered_imdn_list=${MSG_TO}   read_imdn_list=${MSG_TO}
  ${creation_tuid}    ${modification_tuid}=   Run Keyword and Continue on Failure     ValidateCassandra_messages_by_original_folder_timestamp_CHAT    recent=0    seen=1  delivered=1      deliveredimdnlist=${MSG_TO}		readimdnlist=${MSG_TO}
...
DeleteChatMessage
    ...
    Run Keyword and Continue on Failure     Run Keyword If      "${pns_status}" == "PASS"   SendResponse    ${request}  200     OK  ${version}  ELSE    SendResponse    ${request}  400     BadRequest   ${version}
  #  Run Keyword If  ${EnableDeltraSync} == True     Run Keyword and Continue on Failure     ValidateCassandraMessagesAfterDelete    messages=22 messages_by_folder_timestamp=2
...
FolderSearchAfterDeletingtheMsg
    ...
    Run Keyword If      ${EnableDeltraSync} == True     Run Keyword and Continue on Failure     ValidateChatFolderSearchResponse_along_with_Session_Chat    response=${response}        uid=${UID}        msgStatus=Deleted   direction=${DIRECTION}  session_msgStatus=seen        no_of_msgs=${no_of_msgs}     imdn_type=displayed        session_msg_uid=${chat_deposit_uid}
