...
ValidateChatPNSNotfn
	...
	${delivered_list}=	Set Variable If 	'${imdn_pns}' != 'deposit'	${json_out['push-message']['nmsEventList']['nmsEvent'][0]['changedObject']['message']['imdn']['delivered']}
	${read_list}=	Set Variable If		'${imdn_pns}' == 'displayed'	${json_out['push-message']['nmsEventList']['nmsEvent'][0]['changedObject']['message']['imdn']['read']}
...
ValidateCassandraIMDNMapping
    ...
    Switch Connection    cass_db
    Write   select json * from imdnmsgidmapping where userid='${userId}' and creation_tuid=${uid} LIMIT 1 ALLOW FILTERING;
...
ValidateCassandraMessagesAfterDelete    
    ...
    Write   select json * from messages_by_original_folder_timestamp where userid='${userId}' and folderkey='${parent_folder_key}' ALLOW FILTERING;
    ${out}=    Read Until    ${MSTORE_CASSANDRA_KEYSPACE}\>
...
ValidateCassandraMessagesTable
    ...
    Switch Connection    cass_db
    #Write	select json * from messages where userid='${userId}' and folderkey='${message_folder_key}';
    ...
    #select json * from messages where userid='${userId}';
    ...
    log    ${messages}
    ${creation_ts}=     Run     date -d "${messages['creation_ts']}" '+%F %T.%3N'
#    Write	select json * from messages_by_folder_timestamp where userid='${userId}' and folderkey='${parent_folder_key}' and creation_ts='${creation_ts}' and uid=${uid} limit 1;
   ...
   # Write  select json * from messages_by_root_folder_timestamp where userid='${userId}' and folderkey='${message_folder_key}' and creation_tuid=${creation_tuid} limit 1 ;
   # Write  select json * from messages_by_root_folder_timestamp where userid='${userId}' and folderkey='${message_folder_key}' and creation_tuid=${creation_ts} limit 1 ;
    ${out}=    Read Until    ${MSTORE_CASSANDRA_KEYSPACE}\>
    ${res}=    Get Lines Containing String    ${out}    ${user}
...
#    Run Keyword and Continue on failure	   Run Keyword If	'${msg_type}' != 'group'	Should Be Equal As Strings    ${messages['toheader']}       ${to_header}	#ELSE	Should Be Equal As Strings		${messages['toheader']}		${to_members}
   Set Suite Variable      ${modification_tuid}   ${messages['modification_tuid']}
	...
	ValidateCassandraMessagesTableTTLTMO	uid=${creation_tuid}		messages_creation_ts=${messages["creation_ts"]}      message_folder_key=${message_folder_key}     parent_folderkey=${parent_folder_key}
...
ValidateCassandraMessagesTableTTLTMO
    ...
    ${out}=    Read Until    \>
    #Write    SELECT JSON userid,TTL(bodyoctets),TTL(bucketkey),TTL(contenttype),TTL(messagecontext),TTL(contentencoding) from messages_by_root_folder_timestamp  where userid ='${USERID}'' ;
    Write    SELECT JSON userid,TTL(delivered),TTL(folderkey),TTL(imdn_disposition_data),TTL(modification_tuid),TTL(recent),TTL(seen) from messages_by_root_folder_timestamp where userid ='${userId}' and folderkey='${message_folder_key}' and creation_tuid=${uid} ALLOW FILTERING;
     # Write    SELECT JSON userid,TTL(bodyoctets),TTL(bucketkey),TTL(contenttype),TTL(messagecontext),TTL(contentencoding)  from messages_by_root_folder_timestamp where userid ='${userId}' ;
...
ValidateChatFolderSearchResponse
	...
	\    ${obj_resourceURL}=	Set variable	https://${service_root_path}${oem_path}${userId}/objects/${msgFolderkey1}%3d%3a${uid}
	\    ${obj_Folderpath}=		Set Variable	/${RCS_PARENT_PATH}/${msgFolderkey}%3a${uid}
...
ValidateChatFolderSearchResponse_along_with_Session_Chat
    ...
    ${S_resourceURL}=    Set Variable    https://${service_root_path}${oem_path}${userId}/objects/${msgFolderkey1}%3d%3a${uid}
    ...
    ${S_Folderpath}=    Set Variable    /${RCS_PARENT_PATH}/${msgFolderkey}%3a${uid}
...
#    Run Keyword and Continue on failure    Should Be Equal    ${Attributes_pair['content-type'][0]}    ${session_content_type}
    ...
    \   # ${uid}=    Evaluate    ${index} + 1
    \    ${obj_parentFolder}=   Set Variable    https://${service_root_path}${oem_path}${userId}/folders/${msgFolderkey}
    \    ${obj_resourceURL}=    Set variable    https://${service_root_path}${oem_path}${userId}/objects/${msgFolderkey1}%3d%3a${session_msg_uid}
    \    ${obj_Folderpath}=     Set Variable    /${RCS_PARENT_PATH}/${msgFolderkey}%3a${session_msg_uid}
...
ValidateBulkPNSNotification
    [Arguments]    ${pns_headers}    ${pns_body}    ${recipients_uri}=${SUBSCRIBER_ID}    ${folderSyncPath}=${CHAT_PARENT_FOLDER_PATH}    ${oem_path}=${OEM_HOST_URI}    ${service_root_path}=${OEM_SERVER_ROOT_PATH}    ${userId}=${SUBSCRIBER_ID}		${notification_type}=pns
    log    ${pns_body}
...
#    Run Keyword and Ignore Error    Run Keyword and Continue on Failure    Should Match Regexp    ${headers['X-mStoreFE-Addr']}    FEname:${FE_NAME}-nodeId:${NODE_ID}-ConnId:[a-zA-Z0-9]+-slot:${SLOT_ID}-instId:[0-9]+-subOid:[0-9]+-time:[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}Z-localFqdn:${LOCAL_FQDN}
    ...
    Run Keyword and Continue on Failure    Should Be Equal As Strings    ${data['push-message']['nmsEventList']['nmsEvent'][0]['notifyObject']['message']['folderSyncPath']}    ${folderSyncPath}
...
ValidateFTPNSNotfn
    ...
    Run Keyword and Continue on failure    Should Be Equal As Strings  ${len_msg_status}   ${flag_length}  msg="Expectes flag status ${msgStatus} Recieved ${json_out['push-message']['nmsEventList']['nmsEvent'][0]['changedObject']['flags']['flag']}"	msg="pns flag mismatch"

    Run Keyword and Continue on failure    Should Be Equal As Strings    ${json_out['push-message']['nmsEventList']['nmsEvent'][0]['changedObject']['flags']['flag'][0]}    ${status_msg[0]}    ignore_case=True	msg="pns flag mismatch"
    Run Keyword and Continue on failure    Run Keyword If    ${len_msg_status} > 1    Should Be Equal As Strings    ${json_out['push-message']['nmsEventList']['nmsEvent'][0]['changedObject']['flags']['flag'][1]}    ${status_msg[1]}     ignore_case=True	msg="pns flag mismatch"
...
#    Run Keyword and Continue on failure    Should Be Equal As Strings    ${json_out['push-message']['nmsEventList']['nmsEvent'][0]['changedObject']['message']['id']}    ${uid}
    ...
    Run Keyword and Continue on failure    Run Keyword If      '${imdn_pns}' == 'displayed' and '${msg_type}' == 'group'    ValidateImdnDeliveredDisplayed    ${delivered_recipients_uri1}   ${delivered_list}   ELSE IF    '${imdn_pns}' == 'displayed'		Should Be Equal As Strings    ${delivered_list[0]}    ${recipients_uri}

    Run Keyword and Continue on failure     Run Keyword If   '${imdn_pns}' == 'displayed' and '${msg_type}' == 'group'   ValidateImdnDeliveredDisplayed    ${read_recipients_uri1}   ${read_list}   ELSE IF       '${imdn_pns}' == 'displayed'   Should Be Equal As Strings    ${read_list[0]}    ${recipients_uri}
...
ValidateFTFolderSearchResponse
    ...
    \    Run Keyword and Continue on failure    Run Keyword If  '${multipart}' == 'true'	Should Be Equal    ${Attributes_pair['content-type'][0]}   ${FT_CONTENT_TYPE};start="${FT_THUMBNAIL_CONTENT_ID}"		ELSE	Should Be Equal    ${Attributes_pair['content-type'][0]}   ${FT_CONTENT_TYPE}	msg="attribute content-type mismatch foldersync response"
...
ValidateFTFolderSearchResponse_along_with_Session_FT
    [Arguments]     ${response}     ${direction}=Out    ${from_msg}=${MSG_FROM}     ${to_msg}=${MSG_TO}     ${message_context}=${MESSAGE_CONTEXT}   ${msgFolderkey}=${RCS_MESSAGE_FOLDER_KEY}   ${uid}=1    ${msgStatus}=Recent     ${service_root_path}=${OEM_SERVER_ROOT_PATH}    ${oem_path}=${OEM_HOST_URI}     ${userId}=${SUBSCRIBER_ID}  ${no_of_msgs}=2     ${P_ASSERTED_SERVICE}=${ASSERTED_SERVICE}   ${msg_type}=individual    ${sw_acct_id}=${SWIFTACCOUNTID}   ${swift_base_url}=${OPENSTACK_SWIFT_BASE_URL}   ${imdn_type}=deposit        ${read_recipients_uri}=${NONE}  ${delivered_recipients_uri}=${NONE}		${session_msg_uid}=${SESSION_MSG_CTUID}    ${session_msg_context}=${FT_SESSION_MESSAGE_CONTEXT}   ${session_msgStatus}=Recent    ${session_content_type}=Application/X-CPM-Session		${ft_session_asserted_service}=${P_ASSERTED_SERVICE_FT}    ${session_to}=${MSG_TO}	${multipart}=false	${c_uid}=${CREATION_TUIDS}	
    ...
    Run Keyword and Continue On Failure     Should Be Equal As Strings  ${list_of_objects}  ${no_of_msgs}
    ${S_resourceURL}=    Set Variable    https://${service_root_path}${oem_path}${userId}/objects/${msgFolderkey}%3a${session_msg_uid}
    ${S_objectUrl}=    Set Variable    https://${service_root_path}${oem_path}${userId}/objects/${msgFolderkey}%3a${session_msg_uid}
    ${S_parentFolder}=    Set Variable    https://${service_root_path}${oem_path}${userId}/folders/${msgFolderkey}
    ${S_Folderpath}=    Set Variable    /${RCS_PARENT_PATH}/${msgFolderkey}%3a${session_msg_uid}
...
ValidateFTFetchResponse
    [Arguments]    ${response}    ${direction}=Out    ${msgFolderkey}=${RCS_MESSAGE_FOLDER_KEY}    ${msgStatus}=RECENT    ${From}=${MSG_FROM}    ${to_msg}=${MSG_TO}    ${uid}=${CREATION_TUID}    ${service_root_path}=${OEM_SERVER_ROOT_PATH}    ${userId}=${SUBSCRIBER_ID}    ${oem_path}=${OEM_HOST_URI}   ${msg_context}=${MESSAGE_CONTEXT}   ${folder_Path}=${FT_PARENT_FOLDER_PATH}/${userid}/     ${P_ASSERTED_SERVICE}=${ASSERTED_SERVICE}   ${msg_type}=individual	${sw_acct_id}=${SWIFTACCOUNTID}   ${swift_base_url}=${OPENSTACK_SWIFT_BASE_URL}		${imdn_type}=displayed		${multipart}=false	${read_recipients_uri}=${NONE}  ${delivered_recipients_uri}=${NONE}
    ${to}=  Run Keyword If  '${msg_type}' == 'individual'   Create List     ${to_msg}   ELSE    Copy List   ${to_msg}
    ${obj_resourceURL}=    Set Variable    https://${service_root_path}${oem_path}${userId}/objects/${msgFolderkey}%3a${uid}
    ${obj_objectUrl}=    Set Variable    https://${service_root_path}${oem_path}${userId}/objects/${msgFolderkey}%3a${uid}
    ${obj_parentFolder}=    Set Variable    https://${service_root_path}${oem_path}${userId}/folders/${msgFolderkey}
    ...
    Run Keyword and Continue on failure    Run Keyword If    ${len_msg_status} > 1    Should Be Equal    ${data['object']['flags']['flag'][1]}    \\${status_msg[1]}    ignore_case=True
    Run Keyword and Continue on failure    Should Be Equal    ${data['object']['flags']['resourceURL']}    ${obj_resourceURL}/flags
    Run Keyword and Continue on failure    Run Keyword If  '${multipart}' == 'true'    Should Be Equal    ${Attributes_pair['content-type'][0]}   ${FT_CONTENT_TYPE};start="${FT_THUMBNAIL_CONTENT_ID}"      ELSE    Should Be Equal    ${Attributes_pair['content-type'][0]}   ${FT_CONTENT_TYPE}
...
ValidateFTDeletePNSNotfn
    [Arguments]     ${pns_headers}   ${pns_body}   ${userId}=${SUBSCRIBER_ID}    ${direction}=Out   ${msgStatus}=RECENT    ${sender}=${MSG_FROM}    ${recipients_uri}=${MSG_TO}    ${push_recipients_uri}=${recipients_uri}  ${uid}=${CREATION_TUID}   ${pns_subtype}=${PNS_SUBTYPE}   ${service_root_path}=${OEM_SERVER_ROOT_PATH}    ${oem_path}=${OEM_HOST_URI}    ${parentfolderkey}=${RCS_PARENT_FOLDER_KEY}   ${msgFolderkey}=${RCS_MESSAGE_FOLDER_KEY}    ${notification_type}=pns        ${PNS_TYPE}=${PNS_TYPE}  ${msg_type}=individual      ${group_members}=${MSG_TO_GROUP_MEMBERS}	${folder_path}=${FT_PARENT_FOLDER_PATH}/${userId}/ 
...
#    Run Keyword and Continue on failure    Should Match Regexp    ${json_out['push-message']['nmsEventList']['nmsEvent'][0]['deletedObject']['message']['id']}    ${uid}
    Run Keyword and Continue on failure    Should Be Equal    ${json_out['push-message']['nmsEventList']['nmsEvent'][0]['deletedObject']['message']['store']}    ${folder_path} 
    Run Keyword and Continue on failure    Should Be Equal    ${json_out['push-message']['nmsEventList']['nmsEvent'][0]['deletedObject']['message']['objectURL']}    ${pns_object_Url}
    Run Keyword and Continue on failure    Should Be Equal    ${json_out['push-message']['nmsEventList']['nmsEvent'][0]['deletedObject']['message']['direction']}    ${direction}
...
ValidateArchiveTRLdata
	[Arguments]		${trl_data}	 ${no_of_fields}=${HTTPTRLCOUNT}	${store_type}=3		${messageFolderKey}=${RCS_MESSAGE_FOLDER_KEY}	${api_version}=V1	${http_method}=0	${operation_type}=1		${source_node}=RESTCLIENT	${response_code}=201	${mime_content_type}=${CHAT_CONTENT_TYPE}	${content_encoding}=${CHAT_CONTENT_TRANSFER_ENCODING}	${uid}=${CREATION_TUID}		${no_of_msgs}=1		${MSG_CONTEXT_ID}=${TRL_MSG_CONTEXT_ID}	${content_size}=${PAYLOAD_CONTENT_SIZE}		${imdn_status}=${EMPTY}		${content_type}=${BOUNDARY_CONTENT}		${msg_type}=individual		${imdn_trl_correlator}=${X_IMDN_CORRELATOR}
    ...
    Run Keyword and Continue on Failure     Should Be Equal As Strings   ${trl_data[74]}     ${MSG_FROM}		  msg="Generic Message Header Info Caller Address Mismatch i.e, MSG FROM"
    Run Keyword and Continue on Failure     Run Keyword If	'${msg_type}' == 'individual'	 Should Be Equal As Strings   ${trl_data[75]}     ${MSG_TO}		msg="Generic Message Header Info Called Address Mismatch i.e, MSG_TO"
...
ValidateFetchTRLdata
    [Arguments]     ${trl_data}  ${no_of_fields}=198    ${store_type}=${EMPTY}     ${messageFolderKey}=${RCS_MESSAGE_FOLDER_KEY}   ${api_version}=V1   ${http_method}=1    ${operation_type}=2     ${source_node}=RESTCLIENT   ${response_code}=200		${fetch_obj_url}=${OBJECT_URL}
...
ValidateCassandra_messages_by_original_folder_timestamp_LargeMessages
    ...
    ...    ${messagecontext}=${MESSAGE_CONTEXT}    ${deliveredimdnlist}=None    ${readimdnlist}=None  ${msg_from}=${MSG_FROM}   ${msg_to}=${MSG_TO}     ${imdn_msg_id}=${IMDN_MESSAGE_ID}	${msg_type}=individual	${type_msg}=large_msg
    Switch Connection    cass_db
...
#    Run Keyword and Continue on Failure    Should Be Equal As Strings    ${mboft_data['creation_ts']}
#    Run Keyword and Continue on Failure    Should Be Equal As Strings    ${mboft_data['creationdate']}
#    Run Keyword and Continue on Failure    Should Be Equal As Strings    ${mboft_data['direction']}     ${direction_value}
#    Run Keyword and Continue on Failure    Should Be Equal As Strings    ${mboft_data['internaldate']}
#    Run Keyword and Continue on Failure    Should Be Equal As Strings    ${mboft_data['expirytimestamp']}
...
#    Run Keyword and Continue on Failure    Should Be Equal As Strings    ${mboft_data['modification_tuid']}     ${modification_tuid}
    Run Keyword and Continue on Failure    Should Be Equal As Strings    ${mboft_data['rfc2822size']}           ${content_size}
    Run Keyword and Continue on Failure    Run Keyword If	'${type_msg}' != 'session_msg'		Should Match Regexp    ${mboft_data['swiftobjurl']}		/v1/${SWIFTACCOUNTID}/${CONTAINER_INFO['${FT_PARENT_FOLDER_PATH}']}/\\d+:${folderkey}:${creation_tuid}	ELSE	Should Match Regexp    ${mboft_data['swiftobjurl']}		/cassandra/${folderkey}:${creation_tuid}
...
ValidateCassandraMessageActivity
    ...
    Switch Connection    cass_db
    Write   select json * from message_activity where imdnmessageid='${imdnmessageid}' ALLOW FILTERING;
    ${out}=    Read Until    ${MSTORE_CASSANDRA_KEYSPACE}\>
    log    ${out}
...
ValidateFetchTRLdata_C44
    ...
    Run Keyword and Continue on Failure     Should Be Equal As Strings   ${trl_data[27]}    ${fetch_obj_url}      msg="Content Location mismatch"
    Run Keyword and Continue on Failure     Should Match Regexp          ${trl_data[28]}    \\d{16}\\+0000        msg="MsgRetResTs mismatch"
    Run Keyword and Continue on Failure     Should Be Equal As Strings   ${trl_data[29]}     ${store_type}        msg="Retreieve Internal causecode mismatch"
   # Run Keyword and Continue on Failure     Should Be Equal As Strings   ${trl_data[94]}     ${MSTORE_CASSANDRA_IP}     msg="Metadata server Address mismatch"
    Run Keyword and Continue on Failure     Should Be Equal As Strings   ${trl_data[95]}     ${SWIFT_IP}:${SWIFT_PORT}  msg="Storage Object server Address mismatch"
    Run Keyword and Continue on Failure     Should Match Regexp          ${trl_data[96]}     \\d+                       msg="DB Transcation Id mismatch"
    Run Keyword and Continue on Failure     Should Match Regexp          ${trl_data[97]}     \\d+                       msg="Correlation ID mismatch"
